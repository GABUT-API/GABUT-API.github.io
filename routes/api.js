...
var kuhong = db.get('ojan'); // jan diubah
	...
	console.log('WELCOME TO KurrXd API!') // boleh diubah
}

var listkey = ['kurrxkey09', 'kurrxkey'];
var creatorList = ['KurrXd OFFICIAL']; // Nama Lu Ngab
var creator = creatorList[Math.floor(Math.random() * creatorList.length)]; // Ini jan diubah
var key = 'kurrxkey09' // Apikey Lu Ngab
...
router.get('/others/ucapan', async (req, res) => {
    ...
    if (!timeZone) return res.json({ status : false, creator : `KurrXd OFFICIAL`, message : "masukan parameter timeZone"})
    ...
    res.json({
        ...
        console.log(e)
        res.json({ status : false, creator : `KurrXd OFFICIAL`, message : "Eror Bang, Report Cepat Ke Owner"})
    }
...
router.get('/others/hitungmundur', async (req, res) => {
    ...
    if (!bulan) return res.json({ status : false, creator : `KurrXd OFFICIAL`, message : "masukan parameter bulan"})
    if (!tanggal) return res.json({ status : false, creator : `KurrXd OFFICIAL`, message : "masukan parameter tanggal"})
    ...
    res.json({
        ...
        console.log(e)
        res.json({ status : false, creator : `KurrXd OFFICIAL`, message : "Eror, Harap Report Ke Owner"})
    }
})
